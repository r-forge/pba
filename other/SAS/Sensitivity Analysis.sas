...
*********************                  MACRO RUN                       ***********************;
**********************************************************************************************;
%macro sensmac(libname,startover,outset,totalreps,inset,log,depend,exp,indep,miscvar,
...
* runs the logistic regression with the current data set;
	...
	* run a logistic regression model for the corrected misclas data set;
		proc logistic data=misclas descending outest=systematic noprint covout;
			* no class statement used, all variables must be dummy or continuous;
...
%macro randerr(totalreps,outset,inset,j,depend,exp,indep,log,clearout);
	...
	* pull out parameters for conventional analysis graph;
		proc logistic data=&inset descending outest=conventional1 covout noprint;
